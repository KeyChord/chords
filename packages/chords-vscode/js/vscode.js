...
const shouldForceShell = async (file, { shell, cwd, env = process$1.env }) =>
  process$1.platform === "win32" && !shell && !(await isExe(file, cwd, env));
...
const mIsExe = memoize(async (file, cwd, PATH) => {
  const parts = PATH.split(path$1.delimiter)
    .filter(Boolean)
    .map((part) => part.replace(/^"(.*)"$/, "$1"));
  try {
    await Promise.any(
      [cwd, ...parts].flatMap((part) =>
        exeExtensions.map((extension) => access(`${path$1.resolve(part, file)}${extension}`)),
      ),
    );
  } catch {
    return false;
