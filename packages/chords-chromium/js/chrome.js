...
const applyForceShell = async (file, commandArguments, options) =>
  ...
  try {
    await Promise.any(
      [cwd, ...parts].flatMap((part) =>
        exeExtensions.map((extension) => access(`${path.resolve(part, file)}${extension}`)),
      ),
    );
  } catch {
    return false;
...
const getResult = async (nodeChildProcess, { input }, context) => {
  ...
  try {
    await Promise.race([
      onClose,
      ...[instance.stdin, instance.stdout, instance.stderr]
        .filter(Boolean)
        .map((stream) => onStreamError(stream)),
    ]);
...
const getResultError = (error, instance, context) =>
  Object.assign(getErrorInstance(error, context), getErrorOutput(instance), getOutputs(context));
...
const concatenateShell = (file, commandArguments, options) =>
  ...
  try {
    ...
    while (iterators.length > 0) {
      promises = iterators.map(
        (iterator, index) => promises[index] ?? getNext(iterator, index, state),
      );
...
const shouldIgnoreError = (nonIterable, index) =>
  nonIterable.every(Boolean) ? index !== nonIterable.length - 1 : nonIterable[index];
//#endregion
...
const getLocalHosts = () => {
  ...
  for (const _interface of Object.values(interfaces))
    for (const config of _interface) results.add(config.address);
  return results;
};
...
const isLockedPort = (port) =>
  lockedPorts.old.has(port) || lockedPorts.young.has(port) || reservedPorts.has(port);
...
async function createChromiumHandler(appPath) {
  ...
  if (this.enableDebuggingPort) {
    ...
    const hasRemoteDebuggingPort = async (pid) => {
      const { stdout } = await spawn$1("ps", ["-p", pid.toString(), "-o", "command="]);
  ...
  async function handlerJs(code) {
    ...
    await spawn$1(chromeRemoteInterfaceBinPath, [remoteDebuggingPort.toString()], {
      stdin: { string: code },
    });
  }
